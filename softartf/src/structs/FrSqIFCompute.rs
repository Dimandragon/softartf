...
use crate::plotterV2::single_complex_plot;
...
impl FrSqIFCompute{
    pub fn new(planner_in: rustfft::FftPlanner<f32>)->FrSqIFCompute{
        FrSqIFCompute {
    ...
    pub fn one_iter(&mut self){
        ...
        if self.plots == true{
            ...
            name.push_str("inst_freq_before_averaging");
        ...
        if self.max_iters.is_some(){
            if self.max_iters.unwrap()>=self.iters_counter{
                if sum>self.min_freq{
                    ...
                    if self.plots== true{
                        let mut name: String = String::new();
                        name.push_str("signal_before_resampling");
                        name.push_str(& format!("{}", self.iters_counter));
                    ...
                    if self.plots== true{
                        let mut name: String = String::new();
                        name.push_str("signal_after_resampling");
                        name.push_str(& format!("{}", self.iters_counter));
                        let mut path: String = String::new();
                        path.push_str("images/");
                        path.push_str(name.as_str());
                        path.push_str(".png");
                        single_complex_plot(&self.signal_image, &path, Some(name), (1000, 800));
                    }
                    ...
                    if self.plots==true{
                        let mut name: String = String::new();
                        name.push_str("signal_image_before");
                        name.push_str(& format!("{}", self.iters_counter));
                        let mut path: String = String::new();
                        path.push_str("images/");
                        path.push_str(name.as_str());
                        path.push_str(".png");
                        //complex_single_plot(name, &self.signal_image, 1.0, 5.0);
                        single_complex_ignore_plot(&self.signal_image, &path, Some(name), (1000, 800));
                    }

                    if self.plots==true{
                        let mut name: String = String::new();
                        name.push_str("signal");
                        name.push_str(& format!("{}", self.iters_counter));
                        name.push_str(".svg");
                        self.signal.plot(name);
                    }
                    ...
                    if self.plots==true{
                        let mut name: String = String::new();
                        name.push_str("signal_image_after");
