...
pub struct PaddedSignalComplex{
    ...
    pub len: usize,
    pub planner: rustfft::FftPlanner<f32>,
    copy_buffer: Vec<f32>,
...
impl PaddedSignalComplex{
    pub fn new(planner_in: rustfft::FftPlanner<f32>)->PaddedSignalComplex{
        PaddedSignalComplex{
            data: Vec::new(),
            len: 0,
            planner: planner_in,
    ...
    pub fn new_with_len(new_len: usize, planner_in: rustfft::FftPlanner<f32>)->PaddedSignalComplex{
        PaddedSignalComplex{
    ...
    pub fn conv(&mut self, filter: &mut Vec<Complex<f32>>, signal_len: usize, filter_len: usize){
        ...
        let fft = self.planner.plan_fft_forward(self.len*2);
        let ifft = self.planner.plan_fft_inverse(self.len*2);
    ...
    pub fn ht(&mut self){
        self.padding();
        let fft = self.planner.plan_fft_forward(self.data.len());
        let ifft = self.planner.plan_fft_inverse(self.data.len());
        ...
        {
            self.data[i].re=self.data[i].re*2.0/(self.data.len() as f32);
            self.data[i].im=self.data[i].im*2.0/(self.data.len() as f32);
        }
        ifft.process(&mut self.data);
    }
    ...
    pub fn plot(&self, name: String){
        let mut data: Vec<f32> = vec![0.0; self.len];
        for i in 0..self.len{
            data[i] = self[i].re;
        }
        ...
        let temp_len = data.len();
        single_plot(&data, &path, Some(name), (800, 600));
    }
}