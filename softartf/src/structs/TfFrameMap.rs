...
pub struct TfFrameMap{
    ...
    plotting_pow_base: Option<f64>,
    plotting_log_base: Option<f64>,
...
impl TfFrameMap {
    pub fn new(frsqif_computing: &FrSqIFCompute, time_kvant_samples: Option<usize>, time_kvant_seconds: Option<f32>)->TfFrameMap{
        ...
        TfFrameMap{
            ...
            plotting_log_base: Some(2.0),
            plotting_pow_base: Some(1.0),
    ...
    pub fn add_data(&mut self, frsqif_computing: &mut FrSqIFCompute, freq_detail_hz: Option<f32>, freq_detail_period_sec: Option<f32>, time_kvant_samples: Option<usize>, time_kvant_seconds: Option<f32>, inst_freq: &mut Vec<f32>, inst_ampl: &mut Vec<f32>, imf_buffer: &mut Vec<Complex<f32>>){
        ...
        let mut plotting_vec: Vec<&Vec<f32>> = Vec::new();
        ...
        plotting_vec.push(&vec_re);
        plotting_vec.push(&vec_im);
        plotting_vec.push(&vec_ampl);

        let mut path: String = String::new();
        path.push_str("images/");

        path.push_str("inst_ampl_detail");
        path.push_str(& format!("{}", frsqif_computing.iters_counter));
        path.push_str(".png");
        let name: Option<&String> = None;

        crate::plotterV2::plot_lines(&plotting_vec
                                     , &path
                                     , (2000, 1600)
                                     , name
                                     , None);
    ...
    pub fn plot(&mut self){
        if self.plotting_pow_base.is_none(){
            self.plotting_pow_base = Some(1.0);
        }
        if self.plotting_log_base.is_none(){
            self.plotting_log_base = Some(2.0);
        }

        crate::plotterV2::spectrogramm(&self.data, "images/spectrogram_map.png"
                                       , Some("spectrogram_map")
                                       , (5000, 1000), RGBColor{0: 0, 1: 0, 2: 0}
                                       , RGBColor{0: 255, 1: 255, 2: 255}
                                       , self.plotting_pow_base.unwrap()
                                       , self.plotting_log_base.unwrap());
    }
    pub fn set_plotting_pow_base(&mut self, plotting_pow_base: Option<f64>){
        self.plotting_pow_base = plotting_pow_base;
    }
    pub fn set_plotting_log_base(&mut self, plotting_log_base: Option<f64>){
        self.plotting_log_base = plotting_log_base;
    }
