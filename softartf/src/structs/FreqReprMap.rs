...
impl FreqReprMap{
    ...
    pub fn add_data(&mut self, frsqif_computing: &mut FrSqIFCompute, freq_detail_hz: Option<f32>, freq_detail_period_sec: Option<f32>, inst_freq: &mut Vec<f32>, inst_ampl: &mut Vec<f32>, imf_buffer: &mut Vec<Complex<f32>>){
        ...
        let mut plotting_vec = vec![inst_freq.as_ref(), inst_ampl.as_ref()];

        let mut name: String = String::new();
        name.push_str("inst_freq_and_ampl");
        name.push_str(& format!("{}", frsqif_computing.iters_counter));
        let mut path: String = String::new();
        path.push_str("images/");
        path.push_str(name.as_str());
        path.push_str(".png");

        if frsqif_computing.plots == true{
    ...
    pub fn plot(&self)
    {
        ...
        crate::plotterV2::freq_repr_map_plot(&self.data
                                             ,"images/freq_repr_map.png"
                                             ,Some("freq repr")
                                             ,(1000,800));
    }
    ...
    pub fn add_data_fif(&mut self, fif_computing: &mut crate::structs::FIFCompute::FIFCompute, freq_detail_hz: Option<f32>, freq_detail_period_sec: Option<f32>, inst_freq: &mut Vec<f32>, inst_ampl: &mut Vec<f32>, imf_buffer: &mut Vec<Complex<f32>>){
        ...
        let plotting_vec = vec![inst_freq.as_ref(), inst_ampl.as_ref()];

        let mut name: String = String::new();
        name.push_str("inst_freq_and_ampl");
        name.push_str(& format!("{}", fif_computing.iters_counter));
        let mut path: String = String::new();
        path.push_str("images/");
        path.push_str(name.as_str());
        path.push_str(".png");

        if fif_computing.plots == true{
            crate::plotterV2::plot_lines(&plotting_vec
                                         , &path
                                         , (1000, 800)
                                         , Some(name)
