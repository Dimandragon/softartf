...
pub fn my_conv(arr1: &mut [Complex<f32>], arr2: &mut [Complex<f32>], planner:
&mut rustfft::FftPlanner<f32>, filter_len: usize, signal_size: usize, copy_buffer: &mut [Complex<f32>]) {
    ...
    fft_conv(arr1, arr2, planner, filter_len, copy_buffer);
}
...
pub fn fft_conv (arr1: &mut[Complex<f32>], arr2: &mut [Complex<f32>], planner: &mut rustfft::FftPlanner<f32>, filter_len: usize, copy_buffer: &mut [Complex<f32>]) {
    ...
    for i in 0..arr1.len(){
        ...
        arr1[i].re = arr1[i].re/(arr1.len() as f32);
        arr1[i].im = arr1[i].im/(arr1.len() as f32);
        arr2[i].re = arr2[i].re/(arr1.len() as f32);
        arr2[i].im = arr2[i].im/(arr1.len() as f32);
    }
    ifft.process(arr1);
    ifft.process(arr2);
