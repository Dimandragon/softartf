...
pub fn do_t(t: fn(&mut[Complex<f32>]), arr: &mut[Complex<f32>]){
    t(arr);
}
...
pub fn ht (buffer: &mut [Complex<f32>], fft: fn(&mut[Complex<f32>]), ifft: fn(&mut[Complex<f32>])) {
    ...
    {
        buffer[i].re=buffer[i].re*2.0;
        buffer[i].im=buffer[i].im*2.0;
    }
    ifft(buffer);
}
