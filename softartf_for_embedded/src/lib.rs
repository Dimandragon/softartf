...
pub struct FrSqIF{
    pub frsqif_computing: FrSqIFCompute,
...
impl FrSqIF {
    pub fn new(fft_in: fn(&mut[Complex<f32>]), ifft_in: fn(&mut[Complex<f32>])) -> FrSqIF{
        FrSqIF{
            fft: fft_in,
            ifft: ifft_in,
            frsqif_computing: FrSqIFCompute::new(fft_in, ifft_in),
    ...
    pub fn default_computing (&mut self, signal: &[f32], sample_freq: f32){
        ...
        self.set_inst_freq_type_for_resampling(InstFreqTypeForResampling::Extremums);
        self.set_round(Some(0.01));
        self.set_freq_detail_hz(1.0);
        self.set_time_kvant_samples(10);
        self.set_sample_freq(sample_freq);

        self.config_tf_frame_map();
        self.set_inst_freq_type(InstFreqTypeForResampling::Extremums);
...
impl FIF {
    pub fn new(fft_in: fn(&mut[Complex<f32>], &mut[Complex<f32>])) -> FIF{
        FIF{
            fif_computing: FIFCompute::new(fft_in),
            imf_frame: None,
            freq_repr_arr: None,
            freq_repr_map: None,
            tf_frame_arr: None,
